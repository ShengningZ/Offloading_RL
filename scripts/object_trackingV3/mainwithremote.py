...
def remote_filtering(stub, detections, fg_mask):
    ...
    detections_proto = [project_data_pb2.Detection(x1=d[0].item(), y1=d[1].item(), x2=d[2].item(), y2=d[3].item(), confidence=d[4].item(), label=str(int(d[5].item()))) for d in detections]
...
def remote_kalman_filter(stub, kalman_request):
    print("Sending Kalman filter request:")
    print(kalman_request)
    
    response = stub.UpdateState(kalman_request)
    
    print("Received Kalman filter response:")
    print(response)
...
def main():
    ...
    while True:
        ...
        if offload_config['kalman_filter']:
            ...
            detections_proto = [project_data_pb2.Detection(x1=d[0], y1=d[1], x2=d[2], y2=d[3], confidence=d[4], label=str(int(d[5]))) for d in filtered_detections]
