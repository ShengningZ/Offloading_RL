...
class PowerServicer(communication.project_data_pb2_grpc.PowerServiceServicer):
    def __init__(self):
        nvml.nvmlInit()
        self.handle = nvml.nvmlDeviceGetHandleByIndex(0)  # 假设只有一个GPU

    def GetServerPower(self, request, context):
        cpu_power = self.measure_cpu_power()
        gpu_power = self.measure_gpu_power()
        return communication.project_data_pb2.ServerPower(cpu_power=cpu_power, gpu_power=gpu_power)

    def measure_cpu_power(self):
        cpu_percent = psutil.cpu_percent()
        cpu_power = cpu_percent * CPU_RATED_POWER / 100  # 假设 CPU 的额定功率为 CPU_RATED_POWER
        return cpu_power

    def measure_gpu_power(self):
        gpu_power = nvml.nvmlDeviceGetPowerUsage(self.handle) / 1000.0  # 转换为瓦特
        return gpu_power
...
def serve():
    ...
    communication.project_data_pb2_grpc.add_PowerServiceServicer_to_server(PowerServicer(), server)  # 注册PowerServicer
